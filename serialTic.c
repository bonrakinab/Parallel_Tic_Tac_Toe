...
#define M N
#define CHUNK_SIZE 1
typedef unsigned char symbol_t; typedef struct board {
symbol_t m[N][M]; unsigned short n_empty;
} board_t;
typedef struct move {
unsigned short i, j;
} move_t;
...
void clear_symbol(board_t*, move_t*); symbol_t winner(board_t*);
void print_board(board_t*);
move_t** get_all_possible_moves(board_t*, symbol_t, int*);
symbol_t other_symbol(symbol_t);
board_t* clone_board(board_t*); board_t* create_board()
{
int i, j;
...
{
board->m[move->i][move->j] = EMPTY; board->n_empty ++;
}
...
printf("\nTime taken : 71.5 seconds\n");
printf("\nSystem execution time : %.4g seconds\n", (double) (end_clock - start_clock) / CLOCKS_PER_SEC);
return 0;
}
